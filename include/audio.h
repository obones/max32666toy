...
typedef enum _BCLK_SOURCE {
    ...
    MXC_AUDIO_BCLK_SOURCE_F_AUDIO_NM   ///< Clock source = fAudio_Clock *(N/M)
...
/**
 * @brief   Set audio master clock
 *
 * @param   config      Pointer to config struct instance
 *
 * @return  Success/Fail, see \ref MXC_Error_Codes for a list of return codes.
 */
int MXC_AUDIO_SetMasterClock(mxc_audio_I2S_config_t *config);
